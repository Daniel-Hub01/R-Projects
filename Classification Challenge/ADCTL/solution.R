...
# Read the ADCTL CSV file
################################################################################
ADCTL <- read.csv("C:/Users/dtany/Desktop/MAIA_MASTERS_STUDIES/SEM_2_(ITALY)/Statistical_Learning/Challenge_1/ADCTL/ADCTLtrain.csv")
ADCTL <- subset(ADCTL, select = -1)     # Drop the ID column


# LABEL ENCODING: Converting the target (Label) to (AD:1 and CTL:0)
ADCTL$Label <- ifelse(ADCTL$Label == "AD", 1, 0)
ADCTL$Label <- factor(ADCTL$Label)      # encode labels using a two level factor
head(ADCTL)                             # visualize the first 6 rows
# plot(ADCTL[2:10])
...
validation_index <- createDataPartition(y = ADCTL$Label, p = 0.80, list = FALSE) # a list of 80% of the rows for training
training <- ADCTL[validation_index,]    # 80% for training 
validation <- ADCTL[-validation_index,] # 20% for validation
...
ss_pca <- preProcess(training_norm[, -ncol(training)], method = "pca", pcaComp = 50)
...
top10_cols <- head(order(-abs(pca_rot_mat[, 1])), 50) # Get the indices of the top 10 columns used by PCA
...
################################################################################
ADCTL_test <- read.csv("C:/Users/dtany/Desktop/MAIA_MASTERS_STUDIES/SEM_2_(ITALY)/Statistical_Learning/Challenge_1/ADCTL/ADCTLtest.csv")      # loading the test dataset
ADCTLtest_wl <- read.csv("C:/Users/dtany/Desktop/MAIA_MASTERS_STUDIES/SEM_2_(ITALY)/Statistical_Learning/Challenge_1/ADCTL/ADCTLtest_wl.csv") # loading the test dataset with label
test_ids <- ADCTL_test$ID
testwl_ids <- ADCTLtest_wl$ID
ADCTL_test <- subset(ADCTL_test, select = -1)     # Drop the ID column
ADCTLtest_wl <- subset(ADCTLtest_wl, select = -1) # Drop the ID column


# LABEL ENCODING: Converting the target (Label) to (AD:1 and CTL:0)
ADCTLtest_wl$Label <- ifelse(ADCTLtest_wl$Label == "AD", 1, 0)
ADCTLtest_wl$Label <- factor(ADCTLtest_wl$Label)  # encode labels using a two level factor


test_norm <- predict(ss, ADCTLtest_wl)   # Normalization using the same scaling parameters as the training dataset
...
# colnames(test_probs) <- c("CTL:0", "AD:1")
# test_preds <- data.frame(test_ids, test_preds, test_probs)
# test_preds


# write.csv(test_preds, file = "C:/Users/dtany/Desktop/MAIA_MASTERS_STUDIES/SEM_2_(ITALY)/Statistical_Learning/Challenge_1/ADCTL/0075713_ANYIMADU_ADCTLres.csv", row.names = FALSE)
# write.csv(feat_used, file = "C:/Users/dtany/Desktop/MAIA_MASTERS_STUDIES/SEM_2_(ITALY)/Statistical_Learning/Challenge_1/ADCTL/0075713_ANYIMADU_ADCTLfeat.csv", row.names = FALSE)
...
# Plot ROC curve
plot(roc_obj_t, main = "Receiver Operating Characteristic", print.auc_t = TRUE)
...
cat("ADCTLtest AUC:", auc_t, "\n")
cat("ADCTLtest MCC:", MCC_t, "\n")
